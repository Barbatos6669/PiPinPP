...
### GPIO Interrupts (Edge Detection)

Hardware-based interrupt support with callback functions for responsive GPIO event handling:
...
**Features:**
- Non-blocking event detection using libgpiod edge monitoring
- Callbacks execute in dedicated thread (fast response)
- Thread-safe interrupt management
- No polling required - true hardware interrupts
...
int main() {
    ...
    attachInterrupt(22, buttonPressed, FALLING);
...
**Notes:**
- Callbacks should be fast (no blocking operations like `delay()`)
- Maximum one interrupt per pin
- Uses separate monitoring thread per interrupt
...
**Enjoy building with PiPinPP v0.3.7!** 🚀
